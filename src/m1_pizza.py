...
def run_test_draw_points_on_circle():
    ...
    title = 'Tests 4:  5 red on yellow;' + \
            '  10 green1 on unfilled.'
    window = rg.RoseWindow(450, 410, title)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    A=generate_points_on_circle(circle,number_of_points)
    for k in range(len(A)):
        circle = rg.Circle(A[k],10)
        circle.attach_to(window)
        circle.fill_color= color
        point = rg.Point(A[k].x,A[k].y)
...
def run_test_pizza():
    ...
    pizza(window, circle, 5, 'yellow', 4)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    B=generate_points_on_circle(circle,number_of_slices)
    for k in range(len(B)):
        line= rg.Line(circle.center,B[k])
...
def run_test_polygon():
    ...
    title = 'POLYGON tests 3:  10 segments' + \
            ' with thick green lines'
    window = rg.RoseWindow(550, 400, title)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    C= generate_points_on_circle(circle,number_of_segments)
    for k in range(len(C)):
        line= rg.Line(C[k],C[k-1])
        line.attach_to(window)
        line.thickness = thickness
        line.color=color
    window.render(.05)
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  51 lime green lines' + \
            ' on blue circle, hops = 25.'
...
def fancy_polygon(window, circle, number_of_lines,
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    D= generate_points_on_circle(circle,number_of_lines)
    for k in range(len(D)):
        line =rg.Line(D[k],D[k-hops_to_next_point])
        line.arrow='first'
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
